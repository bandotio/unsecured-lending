...
//david
pub fn get_normalized_income(vars: &ReserveData) -> u128{
    // let timestamp = vars.last_update_timestamp; 
    // if timestamp == Self::env().block_timestamp() {
    //     return vars.liquidity_index
    // }
    // let cumulated:u128 = vars.caculate_linear_interest() * &vars.liquidity_index;
    // cumulated
    0
}
//david
fn caculate_linear_interest(vars: &ReserveData) -> u128{
    //let time_difference = Self::env().block_timestamp() - &vars.last_update_timestamp;
    //let interest:u128 = &vars.current_liquidity_rate * time_difference.into() / ONE_YEAR + 1; 
    let interest:u128 = 0;
    interest
}
//done
...
//ting
pub fn update_indexes(vars: &ReserveData,scaled_variable_debt:u128, timestamp:u64) -> (u128, u128){
    // let mut new_liquidity_index = vars.liquidity_index;
    // let mut new_variable_borrow_index = vars.variable_borrow_index;
    // if vars.current_liquidity_rate > 0 {
    //     let cumulated_liquidity_interest = vars.caculate_linear_interest();
    ...
    //         let cumulated_variable_borrow_interest = calculate_compounded_interest(&vars.current_liquidity_rate, timestamp);
...
//ting
fn update_state(vars: &ReserveData){
...
//ting
fn balance_decrease_allowed(vars:&mut BalanceDecreaseAllowedData, asset:AccountId, user:AccountId, amount:u128, user_config:UserReserveData) -> bool{
...
pub fn liquidation_call(vars:&LiquidationCallData, r:ReserveData, collateral:AccountId, borrower:AccountId, debt_to_cover:u128, receive_s_token:bool){
